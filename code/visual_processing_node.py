...
def enter_func(msg):
    ...
    with lock:
        if not image_sub_st:
            image_sub_st = True
            image_sub = rospy.Subscriber('/usb_cam/image_raw', Image, image_callback) #访问摄像头，然后根据target.type处理图像，并把处理好的图像发布到image_pub.
...
if __name__ == '__main__':
    ...
    # 通信服务
    image_pub = rospy.Publisher('/visual_processing/image_result', Image, queue_size=1) #发布处理后的图片，存在ros_image.data
    result_pub = rospy.Publisher('/visual_processing/result', Result, queue_size=1) #发布预测结果msg，比如x centre, y centre, data等
